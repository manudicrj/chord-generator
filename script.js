...
function generateChord() {
    ...
    chord.notes = chord.type.notes.copy();
    chord.add = null;
    chord.sus = null;
    if (randomBoolean(10)) {
        ...
        if (chord.notes.includes(chord.add.note)) {
            ...
            for(var type of types) {
                if (chord.notes.equals(type.notes)) {
                    chord.type = type;
        ...
        chord.sus = sus.getRandom();
        if(chord.notes.includes(chord.sus.note)) {
            chord.sus = null;
        } else {
            chord.notes.push(chord.sus.note);
            for(var type of types) {
                if(chord.notes.equals(type.notes)) {
                    chord.type = type;
                    chord.notes = chord.type.notes;
...
chordEl.onclick = () => {
    ...
    do {
        chord = generateChord();
        i++;
    } while(chord.text != "G<sup>sus</sup>");
    console.log("Generated " + i + " times");
    */
    chord = generateChord();
...
function onMIDIMessage(event) {
    ...
    if(event.data[0]==176) {
        $("#chord").style.fontSize = event.data[2]*2+"px";
    }
